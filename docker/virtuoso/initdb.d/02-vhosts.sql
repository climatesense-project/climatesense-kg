...
-- /trope
--
DB.DBA.VHOST_REMOVE (lhost=>'*ini*', vhost=>'*ini*', lpath=>'/trope');
DB.DBA.VHOST_DEFINE (lhost=>'*ini*', vhost=>'*ini*', lpath=>'/trope', ppath=>'/DAV/', is_dav=>1, is_brws=>0, def_page=>'', vsp_user=>'dba', ses_vars=>0, opts=>vector ('browse_sheet', '', 'cors', '*', 'cors_restricted', 0, 'url_rewrite', 'http_rule_list_9'), is_default_host=>0);
DB.DBA.URLREWRITE_CREATE_RULELIST ('http_rule_list_9', 1, vector ('http_rule_9'));
DB.DBA.URLREWRITE_CREATE_REGEX_RULE ('http_rule_9', 1, '/trope/(.*)', vector ('par_9'), 1, '/fct/rdfdesc/description.vsp?g=http://data.climatesense-project.eu/trope/%U', vector ('par_9'), NULL, NULL, 2, 0, '');

--
-- /persuasion-technique
--
DB.DBA.VHOST_REMOVE (lhost=>'*ini*', vhost=>'*ini*', lpath=>'/persuasion-technique');
DB.DBA.VHOST_DEFINE (lhost=>'*ini*', vhost=>'*ini*', lpath=>'/persuasion-technique', ppath=>'/DAV/', is_dav=>1, is_brws=>0, def_page=>'', vsp_user=>'dba', ses_vars=>0, opts=>vector ('browse_sheet', '', 'cors', '*', 'cors_restricted', 0, 'url_rewrite', 'http_rule_list_10'), is_default_host=>0);
DB.DBA.URLREWRITE_CREATE_RULELIST ('http_rule_list_10', 1, vector ('http_rule_10'));
DB.DBA.URLREWRITE_CREATE_REGEX_RULE ('http_rule_10', 1, '/persuasion-technique/(.*)', vector ('par_10'), 1, '/fct/rdfdesc/description.vsp?g=http://data.climatesense-project.eu/persuasion-technique/%U', vector ('par_10'), NULL, NULL, 2, 0, '');
